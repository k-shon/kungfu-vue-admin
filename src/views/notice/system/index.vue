      ...
      <div style="margin-top: 15px">
        <el-form :inline="true" :model="listQuery" size="small" label-width="140px">
          <el-form-item label="关键字：">
            <el-input v-model="listQuery.productName" class="input-width" placeholder="标题/内容"></el-input>
          </el-form-item>
        </el-form>
      </div>
    </el-card>
      ...
      <el-table ref="newProductTable"
        ...
        <el-table-column label="操作" width="180" align="center">
          <template slot-scope="scope">
            <el-button size="mini"
                       type="text"
                       @click="handleEditSort(scope.$index, scope.row)">查看
            </el-button>
            <el-button size="mini"
                       type="text"
                       @click="handleDelete(scope.$index, scope.row)">标为已读
            </el-button>
          </template>
        </el-table-column>
      </el-table>
...
<script>
  ...
  import {fetchList as fetchProductList} from '@/api/product';
	...
	function getWebSocket(){
			var webSocket = new WebSocket('ws://localhost:1111/socket');//建立与服务端的连接
			//服务端打开连接时
